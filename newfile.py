...
class Engine:
    ...
    async def fragment_score(self, word: str) -> tuple:
        """
        Возвращает (балл, примерная цена строкой).

        Таймаут реализован через asyncio.wait_for, чтобы избежать
        TypeError: unsupported operand type(s) for +: 'ClientTimeout' and 'int'
        в разных версиях aiohttp.
        """
        url = f"https://fragment.com/?query={word}"

        async def _fetch() -> str | None:
            # Сессия создаётся БЕЗ параметра timeout — он несовместим
            # между версиями aiohttp на некоторых хостингах.
        ...
        try:
            # Весь запрос ограничен 10 секундами через asyncio — версия-независимо
        ...
        try:
            ...
            if avg_price >= 500:
                return 30, f"~{avg_price} TON"
            if avg_price >= 100:
                return 20, f"~{avg_price} TON"
            if avg_price > 10:
                return 10, f"~{avg_price} TON"
